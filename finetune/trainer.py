...
from finetune.datasets import I2VDatasetWithResize, T2VDatasetWithResize, MMVDatasetWithResize
...
class Trainer:
    ...
    def __init__(self, args: Args) -> None:
        ...
        mmc_args = {
            ...
            "use_task_emb_text":args.use_task_emb_text,
            "use_rand_drop_inp":args.use_rand_drop_inp,
            "idx_cond_modality":args.idx_cond_modality,
        }


        self.machine_rank2 = self.args.machine_rank
        if self.machine_rank2 >=0:
            self.args.output_dir = self.args.output_dir/f'machine_{self.machine_rank2}'
            os.makedirs(self.args.output_dir, exist_ok=True)
            
        self.mmc_args = mmc_args
        self.task_keys = task_keys
        self.components: Components = self.load_components(task_keys, mmc_args) # Load all pre-trained components.
        self.accelerator: Accelerator = None
        self.dataset: Dataset = None
        self.data_loader: DataLoader = None
        self.save = True # save pipeline
    ...
    def _init_logging(self) -> None:
        ...
        logger.info(f"Accelerator state: \n{self.accelerator.state}", main_process_only=True)
    ...
    def prepare_dataset(self) -> None:
        ...
        if self.args.model_type == "i2v":
            ...
            self.dataset = MMVDatasetWithResize(
                **(self.args.model_dump()),
                device=self.accelerator.device,
                max_num_frames=self.state.train_frames,
                height=self.state.train_height,
        ...
        tmp_data_loader = self.accelerator.prepare_data_loader(tmp_data_loader) # 并行处理数据
        from tqdm import tqdm

        check_cache = self.args.check_cache
        if check_cache:
            for _ in tqdm(tmp_data_loader,desc='preprocess data'): # !!! During iteration, the get_item() function will be called automatically, allowing the dataset to pre-compute related variables in advance.
                ...
            print(f"[INFO] finish cache data!!! exit!!!")
            print(f"[INFO] len(tmp_data_loader): {len(tmp_data_loader)}")
            exit(0)
        else:
            print(f"[INFO] len(tmp_data_loader): {len(tmp_data_loader)}")
            print(f'\n\n\n\n ******** \n [warning] Skip checking cached data.  \n**************\n\n\n\n')
        ...
        print(f"[INFO] len(data_loader): {len(self.data_loader)}")
    ...
    def prepare_trackers(self) -> None:
        ...
        # exist bug
        tracker_name = self.args.tracker_name or "mmgen-experiment"
    ...
    def train(self) -> None:
        ...
        if self.args.save_pipeline:  # save pipeline
            self.__maybe_save_pipeline(global_step,must_save=True)
            exit()
        
        free_memory()
        print(f'wait for all processes to start...')
        self.accelerator.wait_for_everyone()
        ...
        for epoch in range(first_epoch, self.args.train_epochs):
            ...
            for step, batch in enumerate(self.data_loader): 
                ...
                if global_step == 0: # !!! 0 step, we validate the model: for debug
                    self.validate(global_step,self.task_keys, self.mmc_args)
            

                self.task_keys = list(batch['encoded_modals'].keys())

                with accelerator.accumulate(models_to_accumulate):
                    # These weighting schemes use a uniform timestep sampling and instead post-weight the loss
                    loss, dict_log = self.compute_loss(batch) #    
                ...
                freq_print = 100
                if (global_step % freq_print == 0) and (global_step > 0) and (self.accelerator.is_local_main_process):
                    print(f'Training step: {global_step}. Loss: {loss.item()}')
                    print(f"Number of processes: {accelerator.num_processes}")
                    print(f"Current rank: {accelerator.process_index}")
                ...
                if accelerator.sync_gradients:
                    ...
                    self.__maybe_save_pipeline(global_step)

                logs["loss/loss"] = loss.detach().item() 
                logs.update(dict_log)
            ...
            self.accelerator.wait_for_everyone()
            print(f'Finish running one epoch and synchronize...')
            memory_statistics = get_memory_statistics()
    ...
    def validate(self, step: int, task_keys=['rgb'], mmc_args: Optional[Dict[str, Any]] = None) -> None:
        ...
        for i in range(num_validation_samples):
            ...
            validation_artifacts_list = self.validation_step({"prompt": prompt, "image": image, "video": video}, 
                                                             pipe, task_keys, mmc_args)  # validation后的结果
            ...
            extension = "mp4"
            filename = f"validation-{step}-{accelerator.process_index}-{prompt_filename}-{hash_suffix}_merge.{extension}"
            ...
            def add_labels_to_video_grid(video_list, labels, font_scale=0.75, thickness=1, max_per_row=3):
                """
                Stitch multiple videos into a grid layout and add labels below each video.

                :param video_list: List[np.ndarray], each video with shape (T, H, W, 3)
                :param labels: List[str], labels corresponding to each video
                :param font_scale: Font scaling factor
                :param thickness: Font thickness
                :param max_per_row: Maximum number of videos per row
                :return: The stitched video with shape (T, H_total, W_total, 3)
                """

                assert len(video_list) == len(labels), "Mismatch between the number of videos and the number of labels"
            ...
            video_output = add_labels_to_video_grid(combined_array, labels, thickness=2) 
            video_output = [Image.fromarray(frame) for frame in video_output]
            export_to_video(video_output, filename, fps=self.args.gen_fps)
            logger.info(f"Saving validation video to {filename}")
    ...
    def initialize_pipeline(self) -> DiffusionPipeline:
        print("Initializing pipeline...NotImplementedError")
        raise NotImplementedError
    ...
    def __maybe_save_checkpoint(self, global_step: int, must_save: bool = False):
        if self.accelerator.distributed_type == DistributedType.DEEPSPEED or self.accelerator.is_main_process:
            if must_save or global_step % self.args.checkpointing_steps == 0 and self.args.save_checkpoint: 
                # for training
    ...
    def __maybe_save_pipeline(self,global_step: int ,must_save:bool=False):
        #####  Initialize pipeline  #####
        if must_save or global_step % self.args.pipeline_steps == 0 and global_step != 0 : 
            print("[INFO] validate initialize_pipeline()")
            pipe = self.initialize_pipeline()
            import os
            path_dir = os.path.join(self.args.output_dir,"pipeline")
            path = os.path.join(path_dir,f"mmc_{global_step}_pipeline")
            os.makedirs(path, exist_ok=True)
            pipe.save_pretrained(path,max_shard_size="30GB") 
            print(f"[INFO] MMC Pipeline saved to {path}")