...
class Args(BaseModel):
    ...
    #MMC args
    use_decouple_modal: bool = False # support mmc and mmv (visual understanding)
    use_modal_emb_condgen: bool = False # to help distinguish cond/gen modality at input 
    machine_rank: int = -1 # for distributed training
    check_cache: bool = False # whether to use cache for data loading
    
    ### TODO: add support for args below
    use_rand_drop_inp: bool = False # random input drop for augmentation/CFG
    use_task_emb_text: bool = False  #  to further help distinguish differnet tasks using condition tokens

    idx_cond_modality: int = -1 # index of condition modality in the model, -1 means no condition
    ...
    def parse_args(cls):
        ...
        parser.add_argument("--task_keys", type=str, nargs="+", default=["rgb"])
        parser.add_argument('--meta_data',type=str,required=True)
        parser.add_argument("--save_pipeline",type=bool, default=False)
        ...
        parser.add_argument("--pipeline_steps", type=int, default=25000)
        ...
        ### MM related args
        parser.add_argument("--use_decouple_modal", type=lambda x: x.lower() == 'true', default=False)
        parser.add_argument("--use_modal_emb_condgen", type=lambda x: x.lower() == 'true', default=False)
        parser.add_argument("--use_task_emb_text", type=lambda x: x.lower() == 'true', default=False)
        parser.add_argument("--use_rand_drop_inp", type=lambda x: x.lower() == 'true', default=False)


        ## add machine_rank
        parser.add_argument("--machine_rank", type=int, default=-1)

        # check_cache, default=False
        parser.add_argument("--check_cache", type=lambda x: x.lower() == 'true', default=False)

        # condition modality, idx_cond_modality, idx_modal_cond
        parser.add_argument("--idx_cond_modality", type=int, default=-1, help="The index of condition modality")

        args = parser.parse_args()
        print(f"[INFO] __parse_args__ args.task_keys:{args.task_keys}")
