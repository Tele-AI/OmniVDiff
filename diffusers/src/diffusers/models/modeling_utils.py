...
class ModelMixin(torch.nn.Module, PushToHubMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
        ...
        if subfolder=="transformer":
            config['task_keys'] = kwargs.get('task_keys', None)
            config['in_channels'] = 16 * len(config['task_keys'])  # default = 16
        ...
        if from_flax:
            ...
            if low_cpu_mem_usage: # 
                ...
                with accelerate.init_empty_weights():
                    model = cls.from_config(config, **unused_kwargs)  # Construct the model according to the parameters specified in the config.  It first builds the model from the configuration file and then performs initialization.
                if hf_quantizer is not None:
                ...
                if device_map is None and not is_sharded: 
                    ...
                    if subfolder=="transformer":
                        for key in missing_keys:
                            if ('norm_outs' in key or 'proj_outs' in key) and  key not in state_dict:
                                print(f"Initializing missing key: {key}")
                                for i in range(len(config['task_keys'])): 
                                    if str(i) in key:
                                        new_key = key.replace(f"s.{i}","")   
                                        if new_key not in state_dict:
                                            new_key = key.replace(f"s.{i}","s.0")
                                            print(f"[INFO] Expand method!!! Initializing {config['task_keys'][i]} modal missing key use new key : {new_key}")
                                        print(f"[INFO] Initializing {config['task_keys'][i]} modal missing key use new key : {new_key}")
                                        state_dict[key] = state_dict[new_key] 

                            if 'emb_modal_condgen.weight' == key:
                                print(f"Initializing missing key: {key}")
                                weight_shape = model.state_dict()[key].shape 
                                # normal distribution intializatoin
                                weight_init = torch.randn(weight_shape) * 0.02
                                weight_init[0,:] = 0
                                print(f'[INFO] Initializing emb_modal_condgen.weight with zeros except for the first row')
                                state_dict[key] = weight_init

                            if 'task_emb_text' in key: 
                                print(f"[INFO] Initializing missing key: {key}")
                                weight_shape = model.state_dict()[key].shape

                                # normal distribution intializatoin
                                weight_init = torch.randn(weight_shape) * 0.02

                                # TODO: here may need to check, do we need to init to zero
                                state_dict[key] = weight_init
     

                        # Initialize the input embeddings. Because the input embeddings are completely mixed,
                        ...
                        if extra_init: 
                            for key in extra_keys:
                                old_weight = state_dict.get(key,None) 
                                if old_weight is not None:
                                    old_shape = state_dict[key].shape
                                    new_shape = model.state_dict()[key].shape
                                    print(f"[INFO] from_pretrained old_shape:{old_shape},new_shape:{new_shape}")
                                    new_weight = torch.zeros(new_shape)
                                    slices = tuple(slice(0, min(o, n)) for o, n in zip(old_shape, new_shape)) 
                                    print(f"[INFO] from_pretrained slices:{slices}")
                                    new_weight[slices] = old_weight 
                    ...
                    ) # Load the state_dict into the model and distribute it according to the device_map.
