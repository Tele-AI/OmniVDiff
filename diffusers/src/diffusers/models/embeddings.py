...
class CogVideoXPatchEmbed(nn.Module):
    def __init__(
        ...
        if patch_size_t is None:
            ...
            self.proj = nn.Linear(in_channels * patch_size * patch_size * patch_size_t, embed_dim) # 改变输入维度后，需要重新初始化它的模型参数[256,3072]
        ...
        if use_positional_embeddings or use_learned_positional_embeddings:
            ...
            self.register_buffer("pos_embedding", pos_embedding, persistent=persistent) # 可学习的position embeddeding
    ...
    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor):
        ...
        """
        text_embeds = self.text_proj(text_embeds) # torch.Size([2, 226, 3072])
        ...
        if self.patch_size_t is None:
            ...
            # image_embeds,res=(360,480),shape=[2, 14, 64, 45, 60] reshape error ;  image_embeds,res=(240,320),shape=[2, 14, 64, 30, 40] reshape success
